    ...
    public static void profileClick(Button findMatch, Button yourMatches, Button chat, Button prof) {
        ...
        if (Backend.getSelfObject() != null) {
            ...
            for(int i=0;i<1;i++){
                ...
                cb.setSelected(true);
                profile.nationalityBox.getChildren().addAll(cb);
            }
            for(int i=0;i<self.getLanguageList().size();i++){
                CheckBox cb = new CheckBox(self.getLanguageList().get(i));
                cb.setSelected(true);
                profile.languagesBox.getChildren().add(cb);
            }
